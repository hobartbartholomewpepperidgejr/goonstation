...
/mob/living/carbon/human/npc/monkey/tanhony
	...
	New()
		..()
		SPAWN(1 SECOND) // okay this all ought to actually make tanhony spawn in their proper lawyer attire
			if (istype(get_area(src), /area/station/crew_quarters/courtroom))
				src.equip_new_if_possible(/obj/item/clothing/under/misc/lawyer, slot_wear_uniform)
